...
def get_formatter(template_format: str) -> Callable[[Match], str]:
    ...
    def format_struct(match: Match) -> str:
        ...
        try:
            if instance_settings.DATA_TYPES[db_name] is datetime and db_name != 'date':
...
def format_structure(struct: StructureType) -> str:
    """
    ...
    if isinstance(struct, dict):
        ...
        pattern = re.compile(
            r'"(?P<new_name>[a-z28]{2,40})":\s*"(?P<db_name>[a-z28]{2,40})"',
            re.IGNORECASE
        )
    elif isinstance(struct, list):
        formatter = get_formatter("Array")
        pattern = re.compile(r'"(?P<db_name>[a-z28]{2,40})"', re.IGNORECASE)
    else:
        raise InvalidStructure()
...
def get_assurance_query(struct: str) -> str:
    pattern = re.compile(r'c\.(?P<db_name>[a-z28_]+)', re.IGNORECASE)
